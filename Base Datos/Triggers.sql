...
DECLARE
  ...
  estado number(2);
BEGIN
  SELECT estado_produccion INTO estado FROM PEDIDO WHERE id_pedido = :new.id_pedido;
  SELECT descripcion INTO descrip FROM ESTADO_PRODUCCION WHERE ESTADO_PRODUCCION = estado;
...
//FUNCION ENCARGADA DE VALIDAR SI EL TOKEN DEL USUARIO NO HA EXPIRADO Y/O SI ES VALIDO
EN CASO DE SERLO, PERMITE REALIZAR LA ACCIÓN DETERMINADA
...
BEGIN
  ...
  ELSE 
     ...
     SELECT ((to_date(to_char(sysdate, 'yyyyMMddhhmi'), formato_fecha) -
